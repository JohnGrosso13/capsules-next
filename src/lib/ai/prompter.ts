...
export async function generateImageFromPrompt(prompt: string, options: ImageOptions = {}): Promise<string> {
...
export async function editImageWithInstruction(
  ...
  try {
    ...
    console.warn("PNG conversion failed, attempting edit with original format:", (error as Error)?.message);
...
export async function createPostDraft(userText: string): Promise<Record<string, unknown>> {
  ...
  const statusMessage = typeof parsed.message === "string" && parsed.message.trim().length
  ...
  let imagePrompt = typeof postResponse.media_prompt === "string" ? postResponse.media_prompt : null;
...
export async function createPollDraft(userText: string, hint: Record<string, unknown> = {}): Promise<PollDraft> {
  ...
  let question = String(parsed?.poll && (parsed.poll as Record<string, unknown>)?.question || hint.question || "").trim();
  ...
  let options = Array.isArray(parsed?.poll && (parsed.poll as Record<string, unknown>)?.options)
    ...
    ? ((parsed.poll as Record<string, unknown>).options as unknown[]).map((entry) => String(entry ?? "").trim()).filter(Boolean)
  ...
  if (!options.length && Array.isArray(hint.options)) {
    ...
    options = (hint.options as unknown[]).map((entry) => String(entry ?? "").trim()).filter(Boolean);
...
export async function refinePostDraft(
  ...
  const statusMessage = typeof parsed.message === "string" && parsed.message.trim()
  ...
  next.content = typeof postResponse.content === "string" ? postResponse.content.trim() : next.content;
  ...
  const candidatePrompt = typeof postResponse.media_prompt === "string" ? postResponse.media_prompt.trim() : "";
  ...
  const candidateUrl = typeof postResponse.media_url === "string" ? postResponse.media_url.trim() : "";
  ...
  if (editCurrent && base.mediaUrl) {
    ...
    try {
      ...
      const combinedPrompt = [base.mediaPrompt || "", candidatePrompt || userText].filter(Boolean).join(" ");
      ...
      const editedUrl = await editImageWithInstruction(base.mediaUrl, combinedPrompt || userText, {});
...
export async function summarizeFeedFromDB({
  ...
  let suggestionTitle = typeof parsed.suggested_title === "string" ? parsed.suggested_title.trim() : "";
  ...
  let suggestionPrompt = typeof parsed.suggested_post_prompt === "string" ? parsed.suggested_post_prompt.trim() : "";
  ...
  if (!suggestionPrompt) {
    ...
    try {
      ...
      suggestionTitle = typeof fallback.suggested_title === "string" && fallback.suggested_title.trim()
      ...
      suggestionPrompt = typeof fallback.suggested_post_prompt === "string" && fallback.suggested_post_prompt.trim()
  ...
  return {
    ...
    suggestion: suggestionTitle || suggestionPrompt ? { title: suggestionTitle || null, prompt: suggestionPrompt || null } : null,
...
function parseBase64Audio(input: string, fallbackMime: string | null): { buffer: Buffer; mime: string | null } {
...
export async function transcribeAudioFromBase64({
  ...
  const models = Array.from(
    ...
    new Set(
      ...
      [
        ...
        serverEnv.OPENAI_TRANSCRIBE_MODEL,
  ...
  for (const model of models) {
    ...
    try {
      ...
      const transcript = typeof record.text === "string"
