export type PromptIntent = "generate" | "post" | "navigate";
...
const POST_PATTERNS: Array<{ regex: RegExp; confidence: number; reason: string }> = [
...
const NAV_PATTERNS: Array<{ regex: RegExp; confidence: number; reason: string }> = [
...
export function normalizeIntent(intent: string | null | undefined): PromptIntent {
  if (intent === "post" || intent === "navigate") return intent;
  return "generate";
}
