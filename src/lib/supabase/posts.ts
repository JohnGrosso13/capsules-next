...
import { storeImageSrcToSupabase } from "./storage";
import { indexMemory } from "./memories";
...
function resolveValue(row: Record<string, unknown>, metaSources: Array<Record<string, unknown>>, keys: string[]) {
...
export function normalizeLegacyMemoryRow(row: Record<string, unknown>) {
  ...
  const resolvedKind = (resolveValue(row, metaSources, ["kind", "category", "type"]) || row.kind || row.type || row.category || "upload") as string;
...
export async function markPostAttachmentsUnused(
  postRow: { id: string; client_id?: string | null; author_user_id?: string | null; media_url?: string | null },
  deletionTime: string,
) {
  const supabase = getSupabaseAdminClient();
...
export async function persistCommentToDB(comment: Record<string, unknown>, userId: string | null) {
  ...
  const row = {
    ...
    capsule_id: (comment.capsuleId as string) ?? null,
...
export async function createPostRecord(post: Record<string, unknown>, ownerId: string) {
  const supabase = getSupabaseAdminClient();
  const now = new Date().toISOString();
  const draft = { ...post } as Record<string, unknown>;

  const mediaUrl = typeof draft.mediaUrl === 'string' ? draft.mediaUrl.trim() : typeof draft.media_url === 'string' ? (draft.media_url as string).trim() : '';
  ...
  const clientId = typeof draft.id === 'string' && draft.id.trim()
    ? draft.id.trim()
  ...
  const row = pruneNullish({
    ...
    media_url: typeof draft.mediaUrl === 'string' ? draft.mediaUrl : typeof draft.media_url === 'string' ? draft.media_url : null,
    media_prompt: typeof draft.mediaPrompt === 'string' ? draft.mediaPrompt : typeof draft.media_prompt === 'string' ? draft.media_prompt : null,
    user_name: typeof draft.userName === 'string' ? draft.userName : typeof draft.user_name === 'string' ? draft.user_name : null,
    user_avatar: typeof draft.userAvatar === 'string' ? draft.userAvatar : typeof draft.user_avatar === 'string' ? draft.user_avatar : null,
    capsule_id: typeof draft.capsuleId === 'string' ? draft.capsuleId : typeof draft.capsule_id === 'string' ? draft.capsule_id : null,
  ...
  if (isMediaPost) {
    try {
      const prompt = typeof draft.mediaPrompt === "string" ? draft.mediaPrompt.trim() : typeof draft.media_prompt === "string" ? (draft.media_prompt as string).trim() : "";
  ...
  if (upsert.error && (upsert.error.code === 'PGRST204' || (upsert.error.message ?? '').includes("'poll'"))) {
