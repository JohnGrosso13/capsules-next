...
export class ChatEngine {
  ...
  async startGroupChat(
    ...
    try {
      const response = await fetch("/api/chat/groups", {
        ...
        body: JSON.stringify({
          name,
          participantIds: Array.from(unique.keys()),
        }),
      });
      if (!response.ok) {
        let errorMessage = `Failed to create group chat (${response.status})`;
        try {
      ...
      if (options?.activate ?? true) {
        this.store.resetUnread(descriptor.id);
      }
      void this.ensureConversationHistory(descriptor.id);
      return { id: descriptor.id, created: true };
    } catch (error) {
      console.error("ChatEngine startGroupChat failed", error);
      throw error;
    }
  }
  ...
  async addParticipantsToGroup(conversationId: string, targets: ChatParticipant[]): Promise<void> {
    ...
    try {
      const response = await fetch(`/api/chat/groups/${encodeURIComponent(conversationId)}/members`, {
        method: "POST",
        credentials: "include",
        headers: {
          "content-type": "application/json",
      ...
      console.error("ChatEngine addParticipantsToGroup failed", error);
      throw error;
    }
  }
  ...
  async renameGroupChat(conversationId: string, name: string): Promise<void> {
    ...
    try {
      const response = await fetch(`/api/chat/groups/${encodeURIComponent(conversationId)}`, {
        method: "PATCH",
        credentials: "include",
        headers: {
          "content-type": "application/json",
      ...
      if (!response.ok) {
        let errorMessage = `Failed to rename chat (${response.status})`;
        try {
          const payload = (await response.json()) as { message?: string; error?: string };
          errorMessage = payload.message ?? payload.error ?? errorMessage;
        } catch {
      ...
      const descriptor = {
        id: payload.conversationId,
        type: "group" as const,
        title: payload.session?.title ?? trimmed,
        avatar: payload.session?.avatar ?? null,
        createdBy: payload.session?.createdBy ?? null,
        participants: (payload.participants ?? []).map((participant) => ({
          id: participant.id,
      ...
      console.error("ChatEngine renameGroupChat failed", error);
      throw error;
    }
  }
  ...
  async sendMessage(conversationId: string, body: string): Promise<void> {
    ...
    try {
      ...
      if (payload.session) {
        const descriptor = {
          ...
          type:
            ...
            (isGroupConversationId(effectiveConversationId)
              ? ("group" as const)
              : ("direct" as const)),
          title: payload.session.title ?? "",
          avatar: payload.session.avatar ?? null,
      ...
      if (payload?.message && typeof payload.message.id === "string") {
        ...
        if (!isGroupConversationId(effectiveConversationId)) {
  ...
  private applyParticipantsFromDto(
    ...
    const descriptor = {
      id: conversationId,
      type: isGroupConversationId(conversationId) ? ("group" as const) : ("direct" as const),
      title: "",
      avatar: null,
      createdBy: null,
    ...
    if (resolvedId !== conversationId) {
      ...
      if (data.session) {
        const descriptor = {
          ...
          type:
            data.session.type ??
            (isGroupConversationId(resolvedId) ? ("group" as const) : ("direct" as const)),
          title: data.session.title ?? "",
          avatar: data.session.avatar ?? null,
          createdBy: data.session.createdBy ?? null,
  ...
  const payload = event.data as ChatMessageEventPayload;
    ...
    if (payload?.message?.sentAt && !isGroupConversationId(payload.conversationId)) {
      this.recordDirectChannelWatermarkFromIso(payload.message.sentAt);
    }
    if (
      payload &&
