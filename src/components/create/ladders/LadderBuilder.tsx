...
const buildPreviewSnapshot = (state: LadderWizardState, capsuleId: string | null): LadderPreviewSnapshot => {
...
export function LadderBuilder({ capsules, initialCapsuleId = null, previewMode = false }: LadderBuilderProps) {
  ...
  const isTimelineTouched = React.useMemo(() => {
    const schedule = form.schedule;
    const def = defaultScheduleForm;
    const cadence = schedule.cadence ?? "";
    const kickoff = schedule.kickoff ?? "";
    const defCadence = def.cadence ?? "";
    const defKickoff = def.kickoff ?? "";
    return (
  ...
  const guidedCompletion = React.useMemo<Record<GuidedStepId, boolean>>(() => {
    const basicsComplete = {
      ...
      type: guidedVisited.type || Boolean(form.game.title.trim().length),
      ...
      timeline: guidedVisited.timeline || isTimelineTouched,
      ...
      setForm((prev) => {
        ...
        SECTION_KEYS.forEach((key) => {
          ...
          nextSections[key] = {
            ...
            body: toTrimmedString(rawSection.body) || prev.sections[key].body,
