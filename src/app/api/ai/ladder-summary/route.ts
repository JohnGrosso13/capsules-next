...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("billing.ladder_summary.init_failed", billingError);
    return returnError(500, "billing_error", "Failed to verify ladder summary allowance.");
  }
  ...
  try {
    ...
    try {
        ...
        typeof payload?.usage?.total_tokens === "number" && Number.isFinite(payload.usage.total_tokens)
          ...
          : estimateTokensFromText(
              [
                ...
                parsed.data.capsuleName ?? "",
                parsed.data.gameTitle ?? "",
                parsed.data.cadence ?? "",
                parsed.data.rewards ?? "",
      ...
      console.error("billing.ladder_summary.charge_failed", billingError);
      return returnError(500, "billing_error", "Failed to record ladder summary usage.");
    }
