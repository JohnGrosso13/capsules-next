...
async function resolveCapsuleParams(context: LogoParamsContext): Promise<{ id: string }> {
...
export async function POST(req: Request, context: LogoParamsContext) {
  const ownerId = await ensureUserFromRequest(req, {}, { allowGuests: false });
  if (!ownerId) {
    return returnError(401, "auth_required", "Sign in to update capsule logo.");
  }
  ...
  if (!parsedBody.success) {
    return returnError(400, "invalid_request", "Invalid logo payload.", parsedBody.error.flatten());
  }
  ...
  try {
    ...
    return returnError(500, "capsules_error", "Failed to update capsule logo.");
  }
}
