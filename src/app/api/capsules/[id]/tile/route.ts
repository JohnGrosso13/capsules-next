...
import { updateCapsulePromoTileImage } from "@/server/capsules/service";
...
type TileParamsContext = {
  params: { id: string } | Promise<{ id: string }>;
};

async function resolveCapsuleParams(context: TileParamsContext): Promise<{ id: string }> {
...
export async function POST(req: Request, context: TileParamsContext) {
  const ownerId = await ensureUserFromRequest(req, {}, { allowGuests: false });
  if (!ownerId) {
    return returnError(401, "auth_required", "Sign in to update capsule promo tile.");
  }
  ...
  if (!parsedBody.success) {
    return returnError(400, "invalid_request", "Invalid tile payload.", parsedBody.error.flatten());
  }
  ...
  try {
    ...
    if (!finalUrl) {
      throw new Error("Tile upload did not produce a URL");
    }

    const result = await updateCapsulePromoTileImage(ownerId, parsedParams.data.id, {
      tileUrl: finalUrl,
      storageKey: finalKey,
      mimeType,
    ...
    console.error("capsules.tile.update error", error);
    if (error instanceof Error && "status" in error) {
      const status = Number((error as { status?: number }).status) || 500;
      return returnError(status, "capsules_error", error.message);
    }
    return returnError(500, "capsules_error", "Failed to update capsule promo tile.");
  }
}
